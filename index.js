...
console.log("task3", getArtistByIndex[0]);
...
function removeArtist(arr, index) {
  const newArtistArr = arr.slice();
  newArtistArr.splice(index, 1);
  return newArtistArr;
}

console.log("task5", removeArtist(artists, 1));
...
console.log("Robert","1977-2022", "Web Design", "American", 'I have a background in customer service at Big Retail Chain. I am attending BloomTech to become a Frontend Developer.');
...
function artistByCountry(arr, nationality){
  ...
  for (let i = 0; i < arr.length; i++){
    if (arr[i].nationality === nationality){
      nationalityArr.push(arr[i].name)
    }
  }
  return nationalityArr;
}

console.log("task 9", artistByCountry("Spanish"));
